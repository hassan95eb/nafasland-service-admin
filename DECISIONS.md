...
**ریسک باقی‌مانده:** مستندات بررسی‌شده فقط بخش عمومی (خواندنی) API است؛ اندپوینت‌های مدیریتی ایجاد/ویرایش/حذف در آن نیستند. باید تأیید شود که API مدیریتی هم همین `version` را برمی‌گرداند و با هر تغییر آن را به‌روز می‌کند.
...
- تأیید اینکه API **مدیریتی** پرتال هم فیلد `version` را برمی‌گرداند و با هر ویرایش به‌روزش می‌کند (مستندات عمومی فقط بخش خواندنی را پوشش می‌دهد).
