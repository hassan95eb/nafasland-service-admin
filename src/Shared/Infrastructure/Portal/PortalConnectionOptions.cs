...
/// <summary>
/// The part of the <c>Portal</c> config section every module that talks to the
/// portal shares (ADR-054): connection, rate limit (ADR-026) and resilience
/// (ADR-050). Module-specific keys in the same section (e.g. Catalog's
/// <c>TestProductId</c>) live in that module's own options class, so config
/// keys and <c>Portal__*</c> environment variables are unchanged.
/// </summary>
public sealed class PortalConnectionOptions
{
